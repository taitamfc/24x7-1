...
(function($) {
    ...
    function MasonryLayout(element) {
        ...
        if ($('body').hasClass('blog-three-columns'))
            this.columnCount = 3;
        if ($('body').hasClass('blog-four-columns'))
            this.columnCount = 4;
        if ($('body').hasClass('blog-five-columns'))
            this.columnCount = 5;
    ...
    var _initComponents = function(container) {
        ...
        if ($.fn.flexslider) {
            $('.flexslider:not(.wpb_flexslider)', container).each(function() {
                ...
                try {
                    config = $.extend(config, JSON.parse('{' + slider.attr('data-slider-config') + '}'));
                }
                catch (e) {
                }
